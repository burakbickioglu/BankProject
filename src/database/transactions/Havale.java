...
public class Havale extends DbConnection implements IBilgiController {
    ...
    public boolean havaleYapildiMi() {
        if (bilgilerGecerliMi()) {
            String query = "UPDATE kullanici_bakiye set bakiye = bakiye - '" + this.gonderilecekMiktar + "' "
                    + "where kullanici_id = '" + getHesapBilgileri().getKullaniciId() + "'";

            String query2 = "UPDATE kullanici_bakiye set bakiye = bakiye + '" + this.gonderilecekMiktar + "' "
